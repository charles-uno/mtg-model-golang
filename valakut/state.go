...
func (s *state) next() []state {
    ...
    states = append(states, s.clone_play_mountain()...)
    states = append(states, s.clone_cast_explore()...)
    states = append(states, s.clone_cast_sakura_tribe_elder()...)
...
func (s *state) clone_play_mountain() []state {
    card := slug("Mountain")
    if !s.in_hand(card) || s.lands == 0 { return []state{} }
    clone := s.clone()
    clone.note("Playing " + card)
...
func (s *state) clone_cast_sakura_tribe_elder() []state {
    card := slug("Sakura-Tribe Elder")
    cost := 2
    if !s.in_hand(card) || !s.can_pay(cost) { return []state{} }
    clone := s.clone()
    clone.note("Casting " + card)
    clone.cast_sakura_tribe_elder()
    return []state{clone}
}

func (s *state) clone_cast_primeval_titan() []state {
    card := slug("Primeval Titan")
    cost := 6
    if !s.in_hand(card) || !s.can_pay(cost) { return []state{} }
    clone := s.clone()
    clone.note("Casting " + card)
...
func (s *state) play_forest() {
    card := slug("Forest")
    s.hand = remove(s.hand, card)
    s.board = append(s.board, card)
    s.mana += 1
    s.lands -= 1
}

func (s *state) play_mountain() {
    card := slug("Mountain")
    s.hand = remove(s.hand, card)
    s.board = append(s.board, card)
    s.mana += 1
...
func (s *state) cast_explore() {
    s.hand = remove(s.hand, slug("Explore"))
    s.pay(2)
    s.lands += 1
    s.draw(1)
}

func (s *state) cast_sakura_tribe_elder() {
    s.hand = remove(s.hand, slug("Sakura-Tribe Elder"))
    s.pay(2)
    s.board = append(s.board, slug("Forest"))
}

func (s *state) cast_primeval_titan() {
    s.hand = remove(s.hand, slug("Primeval Titan"))
    s.pay(6)
    s.done = true
}
